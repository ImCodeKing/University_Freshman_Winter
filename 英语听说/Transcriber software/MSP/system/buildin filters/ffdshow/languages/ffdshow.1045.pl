...
ffdshow default=Domyœlne ffdshow
...
;
Select name of exported Girder file=Wybierz nazwê eksportowanego pliku Grider'a
;
...
Names of DirectShow filters, wildcard allowed=Nazwy filtrów DirectShow, symbole wieloznaczne dozwolone
...
halfpel on all MB types, qpel on the winner=halfpel na wszystkich typach MB, qpel na zwyciêzcy
...
Flat=Macierz p³aska
JVT=Macierz JVT
Extended AC3 decoder (Dolby Digital Plus). Works only if AC3 decoding is enabled!=Dekoder Extended AC3 (Dolby Digital Plus). Dzia³a tylko, jeœli dekodowanie AC3 jest w³¹czone!
Dolby TrueHD/MLP decoder. Works only if AC3 decoding is enabled!=Dekoder Dolby TrueHD/MLP. Dzia³a tylko, jeœli dekodowanie AC3 jest w³¹czone!
...
FLAC (Free Lossless Audio Codec). To play .flac files you also need a source filter.=FLAC (Free Lossless Audio Codec). By odtwarzaæ pliki .flac potrzebujesz równierz filtra Ÿród³a.
TTA (Lossless Audio Codec). To play .tta files you also need a source filter.=TTA (Lossless Audio Codec). By odtwarzaæ pliki .tta potrzebujesz równierz filtra Ÿród³a.
Linear=Liniowa
Cubic=Szeœcienna
Top field first=Górnego pola
...
Most TV and Projectors use this.\nConsult the manual of your device.=Wiêkszoœæ TV czy Projektorów u¿ywa tego.\nUwzglêdnij informacje w instrukcji twojego urz¹dzenia.
YCbCr <-> RGB conversion options=Opcje konwersji YCbCr <-> RGB
Force weave=Wymuœ weave
Force bob=Wymuœ bob
Auto: according to source flags, bob for interlaced frames, weave for progressive frames.\nForce weave: weave for each frame, regardless of source flags.\nForce bob: bob for each frame, regardless of source flags.\n\nNote: In most cases, Weave is the same as unchecked "Set interlace flag in output media type".=Auto: zgodnie z oznaczeniem w Ÿródle, bob dla klatek z przeplotem, weave dla klatek progresywnych.\nWymuœ weave: weave dla ka¿dej klatki, bez wzglêdu na oznaczenie w Ÿródle.\nWymuœ bob: bob dla ka¿dej klatki, bez wzglêdu na oznaczenie w Ÿrodle.\n\nUwaga: W wiêkszoœci przypadków, Weave jest tym samym czym odznaczenie opcji "Ustaw flagê przeplotu w strumieniu wyjœciowym".
ffdshow searches subtitle files in the folders which are configured in the edit box above.\nFor video.avi, ffdshow searches video.utf, video.idx, video.sub,... and use the file which is found at the first time.\nEnumerate extensions in the order you like and separate them by semicolons.\n\nutf;idx;sub;srt;smi;rt;txt;ssa;aqt;mpl;usf is the default settings.=ffdshow szuka plików z napisami w folderze ustawionym w opcji wy¿ej.\nDla plików video.avi, ffdshow szuka video.utf, video.idx, video.sub,... oraz u¿ywa pliku który zosta³ znaleziony jako pierwszy.\nWypisz rozszerzenia w porzadku jakim lubisz i oddziel je œrednikami.\n\nutf;idx;sub;srt;smi;rt;txt;ssa;aqt;mpl;usf jest domyœlnym ustawieniem.
Experimental, may give garbled subtitles.=Opcja eksperymentalna, mo¿e powodowaæ zniekszta³cenie napisów.
...
Cubic interpolation=Szeœcienna interpolacja
H.264 and MPEG-1/2 decoder only=Tylko dekoder H.264 oraz MPEG-1/2
Checked: If soft telecine is detected, frames are flagged as progressive.\n\nYou may want to unckeck if you have interlaced TV.=Zaznaczone: Jeœli ³agodne Telekino zostanie wykryte, klatki bêd¹ traktowane jako progresywne.\n\nMo¿esz to odznaczyæ, jeœli posiadasz TV z przeplotem.
equal number of frames, equal framerate=niezmienionej iloœci klatek + niezmienionej iloœci szybkoœci klatek
double number of frames, double framerate=podwojonej iloœci klatek + podwojonej iloœci szybkoœci klatek
double number of frames, equal framerate=podwojonej iloœci klatek + niezmienionej iloœci szybkoœci klatek
...
Framerate doubler=Podwajacz iloœci klatek
Scan of bars will stop after this position. Set 0 to always scan for bars=Skaner zatrzyma siê po tym ustawieniu. Ustaw 0 by zawsze skanowaæ
Tolerance when scanning bars=Tolorancja podczas skanowania
Set the delay in ms to rescan for the bars=Ustaw opóŸnienie w ms do ponownego przeskanowania
...
1021=Zasiêg:
1023=Rozmywanie:
1143=aWarpSharp implementuje oryginalne wyostrzanie wysokiej jakoœci. "Oryginalne" oznacza, ¿e algorytm ró¿ni siê w pewnym stopniu od innych filtrów wyostrzaj¹cych. Kod jest w pe³ni zoptymalizowany dla iSSE z wysok¹ dok³adnoœci¹.\r\n\r\nMarc Fauconneau
0=WarpSharp
1338=WarpSharp
...
1920=Kwantyzator:
1143=Filtr ten jest eksperymentalnym filtrem, który dla ka¿dego bloku 8x8 wykonuje dyskretn¹ transformatê kosinusow¹ (DCT), skaluje wybran¹ czêstotliwoœæ i odwraca proces wykonuj¹c odwrotn¹ dyskretn¹ transformatê kosinusow¹ (IDCT).\r\n\r\nPrawa autorskie (C) 2002 Tom Barry - trbarry@trbarry.com
...
1790=Wsparcie dla znaczników SSA w plikach SRT oraz znaczników HTML w plikach SSA
...
2248=Wykryj ³agodne Telekino oraz œredni czas klatki
...
[246]
0=Filtr SOI
1686=Filtr o skoñczonej odpowiedzi impulsowej
1688=Rz¹d filtru:
...
1692=Okno:
1683=Poka¿ widmo
1143=W przetwarzaniu sygna³u wystêpuje wiele przypadków, w których sygna³ wejœciowy do systemu zawiera dodatkow¹ niepotrzebn¹ zawartoœæ oraz szum, który mo¿e obni¿yæ jakoœæ po¿¹danego fragmentu. Dlatego w takich przypadkach mo¿emy usun¹æ lub przefiltrowaæ niepotrzebny sample. Dla przyk³adu, w systemach telefonicznych nie ma potrzeby by transmitowaæ bardzo wysokich czêstotliwoœci, gdy¿ wiêkszoœæ mowy wypada w przedziale od 400 do 3.400 Hz. Dlatego wszystkie czêstotliwoœci powy¿ej lub poni¿ej tego przedzia³u zostaj¹ odfiltrowane. Zakres czêstotliwoœci pomiêdzy 400 i 3.400 Hz, który nie zosta³ odfiltrowany znany jest jako pasmo przepustowe, a czêstotliwoœæ która zosta³a zablokowana znana jest jako stopband.\r\n\r\nSOI, Filtr o skoñczonej odpowiedzi impulsowej jest jednym z g³ównych typów filtrów u¿ywanych w Cyfrowym Przetwarzaniu Sygna³ów. O filtrach SOI mówi siê, ¿e s¹ skoñczone, poniewa¿ nie posiadaj¹ ¿adnego sprzê¿enia zwrotnego. Zatem, jeœli wyœlesz jeden pojedynczy impuls przez system, to wyjœcie bêdzie niezmiennie darzyæ do zera tak szybko jak impuls biegnie przez filtr.
...
1745=Tylny prawy
1744=LFE
1143=Kana³y mog¹ byæ tak¿e zamienione przy pomocy filtra Mikser, ale ten jest szybszy.
...
1143=Dla lepszej wydajnoœci na wolniejszych komputerach b¹dŸ w przypadku niekompatybilnych sterowników, odznacz wszystko z wyj¹tkiem opcji "16 bit sta³oprzecinkowy".\r\nOpcja LPCM jest przeznaczona do pod³¹czania filtrów dekoduj¹cych audio, jak np. filtr Cyberlink'a.
2234=Przejœcie przez S/PDIF oraz HDMI:
...
1422=Minimalny odstêp klatki I:
1424=Klatki B
...
[2244]
0=Opcje dekodera
2245=Zastosuj dynamiczne pasmo kompresji, jeœli dostêpne (dekodery AC3 oraz E-AC3)

[296]
2222=Zacznij od:
2224=Podwojenie liczby klatek
2223=Pomiñ sprawdzanie przestrzennego przeplotu

[241]
1528=Wyostrz
1529=Dwustronnie
1531=Po³¹cz luminancyjn¹ i chrominancyjn¹ matrycê ruchu
1530=Zmapuj
1526=Próg:
